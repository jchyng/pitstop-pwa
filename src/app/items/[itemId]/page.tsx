...
import { getMileage, getLogs, migrateLogsIfNeeded, mergeItemWithCustom, getCustomInterval, getHiddenItems, hideItem, unhideItem } from '@/lib/storage';
  ...
  useEffect(() => {
    ...
    fetch(`/cars/${savedCarId}.json`)
      ...
      .then((data: CarData) => {
        ...
        setIsHidden(getHiddenItems(savedCarId).has(itemId));
        setIsLoading(false);
      });
  }, [itemId]);
      ...
      {item && (
        ...
        >
          ...
          <button
            onClick={() => {
              if (isHidden) {
                unhideItem(carId, itemId);
                setIsHidden(false);
              } else {
                hideItem(carId, itemId);
                setIsHidden(true);
                router.back();
              }
            }}
            style={{
              ...
              fontSize: 13,
              fontWeight: 500,
              color: isHidden ? 'var(--color-nav-active)' : 'var(--color-text-muted)',
              cursor: 'pointer',
              fontFamily: 'var(--font)',
              display: 'flex',
              alignItems: 'center',
