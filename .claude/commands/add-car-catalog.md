...
- 총 페이지 수를 확인한다.
- 다음 키워드가 등장하는 페이지를 찾는다: `정비`, `교체`, `점검`, `주기`, `km`, `개월`, `엔진오일`, `브레이크`, `타이어`, `냉각수`, `점화`, `에어필터`, `변속기`.
...
- 이미지의 표/문자에서 각 소모품 항목의 교체 주기(km, 개월)를 최대한 읽어낸다.
- 읽을 수 없는 항목은 같은 브랜드의 유사 차종 JSON을 참고하여 합리적으로 채운다.
